...
2. On driver change, it immediately posts to the server and the server sends notifications.
3. On fuel update, it posts current fuel level and estimated time remaining.
4. Server handles deduplication, so all teammates can run the app simultaneously and only one notification fires.
5. App runs as a system tray icon. Double-click to open the dashboard.
...
|---|---|
| `main.py` | Entry point |
| `config.py` | Settings and stored credentials |
| `api_client.py` | HTTP calls to server |
| `iracing_monitor.py` | Reads iRacing shared memory |
| `gui/login.py` | Login window for first run |
| `gui/tray.py` | System tray and dashboard window |
| `gui/reference_lap_selector.py` | Coaching reference lap picker and upload UI |
